...
tests <- read.sas7bdat('./Data/dw1stcovtest2020_6_15.sas7bdat')
...
plot(ds3$date, (ds3$new.cases), main='New cases, based on report date', xlab='Date', ylab='Reported cases', bty='l', type='l')
polygon( as.Date(c( c('2020-03-17','2020-03-24'), c('2020-03-24','2020-03-17'))), c(c(-1000,-1000 ),c(1000,1000)),col = rgb(0, 0, 1, alpha = 0.1), border = NA)
abline(v=seq.Date(from=as.Date('2020-03-17'),length.out=15, by='week') , lty=2, col=rgb(0,0,0,alpha=0.1))

plot(ds3$date, log(ds3$new.cases), main='log(New cases, based on report date)', type='l', xlab='Date', ylab='Log Reported cases', bty='l')
polygon( as.Date(c( c('2020-03-17','2020-03-24'), c('2020-03-24','2020-03-17'))), c(c(-1000,-1000 ),c(1000,1000)),col = rgb(0, 0, 1, alpha = 0.1), border = NA)
abline(v=seq.Date(from=as.Date('2020-03-17'),length.out=15, by='week') , lty=2, col=rgb(0,0,0,alpha=0.1))


#Pos tests
plot(ds3$date, ds3$pos.tests, main='New cases, based on test date', xlab='Date', ylab='Log(cases)', bty='l' , type='l')
polygon( as.Date(c( c('2020-03-17','2020-03-24'), c('2020-03-24','2020-03-17'))), c(c(-1000,-1000 ),c(1000,1000)),col = rgb(0, 0, 1, alpha = 0.1), border = NA)
abline(v=seq.Date(from=as.Date('2020-03-17'),length.out=15, by='week') , lty=2, col=rgb(0,0,0,alpha=0.1))

plot(ds3$date, log(ds3$pos.tests), main='Log(New cases, based on test date)', xlab='Date', ylab='Log(Reported Cases)', bty='l', type='l')
polygon( as.Date(c( c('2020-03-17','2020-03-24'), c('2020-03-24','2020-03-17'))), c(c(-1000,-1000 ),c(1000,1000)),col = rgb(0, 0, 1, alpha = 0.1), border = NA)
abline(v=seq.Date(from=as.Date('2020-03-17'),length.out=15, by='week') , lty=2, col=rgb(0,0,0,alpha=0.1))
...
Comparing the new cases of COVID-19 based on test date or date of report, we can see a long lag early in the epidemic between test date and report date. This gap seems to narrow as the epidemic progresses, perhaps due to improvements in testing
```{r}
plot(ds3$date, (ds3$new.cases), main='Test date (red) vs report date (black)', xlab='Date', ylab='(Reported cases)', bty='l', type='l')
points(ds3$date, (ds3$pos.tests), col='red', type='l')
```
...
-We evaluate three epidemiological time series: COVID-19 hospitalizations, number of cases of covid-19 (based on date of test), and number of cases of covid-19 (based on date of report). 
...
abline(h=0)
```
### Distributed lags for the association between viral RNA in sewage and new reported COVID-19 cases, based on reporting date
...
Cumulative relationship between viral RNA in sewage and new cases. This shows that lags of sewage data at longer time lags (6-8 days) best correlate with the time series of reported cases, with considerable uncertainty in the estimates. +/-90% Credible Intervals.
```{r, eval=T}
...
### Distributed lags for the association between viral RNA in sewage and number positive, based on test date. 
...
Y2 <- ds.mod.c[,"pos.tests_R1, N1"]
Y2[is.nan(Y2)] <-NA

log.offset2 <- log(ds.mod.c[,"cov.tests_R1, N1"])
...
This shows that lags of 0-2 days are associated with the case time series, based on date of test, (on the cumulative plot, beta does not increase after 2 days) +/-90% Credible Intervals. From the plots, the sewage sludge RNA concentration increase preceded the increase in number of cases, based on test date.
...
Cumulative relationship between viral RNA in sewage and number of cases based on test date. +/-90% Credible Intervals.
