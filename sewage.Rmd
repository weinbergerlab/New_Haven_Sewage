...
```{r}
mod2 <- mod2.func(W=scale.W1, Y=Y1)
...
```{r}
times <- 1:nrow(mod2$rw.x)
yrange=range(c(exp(W1),exp(as.matrix(mod2$rw.x ))), na.rm=T)
plot(times, exp(mod2$rw.x[,1]), type='l', bty='l', ylim=yrange, ylab='Viral concentration')
polygon(c(times, rev(times)),  c(exp(mod2$rw.x[,2]), rev(exp(mod2$rw.x[,3]))) ,col = rgb(0, 0, 1, alpha = 0.1), border = NA ) 

matplot(exp(W1), add=T, type='p')
...
t <- 1:nrow(mod1$X1)
ylim1 <- range(c(exp(mod1$X1),(scale.W1)), na.rm=T)
matplot(t, exp(mod1$X1), col='blue', main='Smoothed Viral concentration', type='l',lty=c(1,2,2), bty='l', ylim=ylim1, ylab='Viral concentration')
matplot(exp(scale.W1), add=T, type='p')
...
Seem to be some convergence issues with this model that need to be resolved, possibly due to missing cases data
...
```{r, eval=F}
mod2 <- mod2.func(W=scale.W2, Y=Y2)
