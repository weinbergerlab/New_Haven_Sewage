...
yrange=range(c((W1),(as.matrix(mod2$rw.x ))), na.rm=T)
plot(times, (mod2$rw.x[,1]), type='l', bty='l', ylim=yrange, ylab='Scaled-log-Viral concentration')
polygon(c(times, rev(times)),  c((mod2$rw.x[,2]), rev((mod2$rw.x[,3]))) ,col = rgb(0, 0, 1, alpha = 0.1), border = NA ) 
